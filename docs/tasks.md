...
9. **`docs/TEST-RUN/TEST-RUN-MASTER-001.md`** — 정무님용 **단일 운영 리허설 런북**(storefront→입금→allocation·payable·선택 가격정책·취소). 브라우저 위→아래 순서.  
10. **`docs/TEST-DEV/TEST-RUN-001.md`** — 상세 손 실행·SQL·환경 확인(구 `docs/TEST-RUN-001.md` 경로).  
11. **`docs/TEST-DEV/TEST-RUN-PRICING-001.md`** — storefront **가격 정책·품목 스냅샷** 손 검증(`pricing_policies`·우선순위·폴백·immutable·기간·`admin_logs`). **`TEST-RUN-ERP-001`** 와 병행 시 **품목 단가·스냅샷 선확인** 권장.  
12. **`docs/TEST-DEV/TEST-RUN-ERP-001.md`** — storefront→ERP **회계 숫자 정합성** 손 검증(수금·allocation·`supplier_payables`·중복·취소·fee·RLS·`admin_logs`). TEST-RUN-001 완료 후 권장.  
13. **`docs/PAYMENT-FORENSIC-001.md`** — storefront **결제·주문** 실구현 범위(Toss·무통장·카카오·`commerce_orders`) 포렌식; `COMMERCE-FLOW.md` 대비 코드 갭 명시.  
14. **`docs/ORDER-FORENSIC-001.md`** — 공급자 **`orders`/`order_lines`** 주문등록·수금동시처리·`point_used`/할인·거래처 연결·`order_edit_lock_days` 수정제한 포렌식; `commerce_orders`와 구분.  
15. **`docs/ORDER-LOCK-FORENSIC-001.md`** — `order_edit_lock_days`가 **`settings` vs `admin_settings`** 중 어디를 읽는지·잠금 bypass 여부·운영 영향(CASE) 정리.  
16. **`docs/POINT-FORENSIC-001.md`** — `orders.point_used`·적립금 UI가 **실제 잔고 시스템인지** vs **헤더 할인 숫자인지**; 테이블·원장·취소·중복 사용 포렌식.  
17. **`docs/DISCOUNT-FORENSIC-001.md`** — 기간할인·프로모션·`commerce_price`/`original_price`·storefront 계산·고객별 타깃·export 갭 포렌식.  
18. **`docs/DISCOUNT-ENGINE-DESIGN-001.md`** — DISCOUNT-ENGINE-DESIGN-001: B2B 가격정책 엔진 설계(고객 청구·공급자 기준·부담 주체별 할인·스냅샷·ERP 연동; 구현·migration 아님). 수수료 기준 **안 A/B** 등 탐색안은 있으며, **정책 확정은 `DECISIONS.md` [D-020]** 가 우선한다.  
19. **`docs/PRODUCT.md` §5·§10·§12·§13 + `docs/CONTEXT.md` [ARCH-08A]·[ARCH-09]** — STOREFRONT-ARCH-001: 디닷페이스 플랫폼 주문·ERP·fulfillment(해내음)·정산 축 문서 정렬.  
20. **`docs/PLATFORM-ERP-ARCH-001.md`** — PLATFORM-ERP-ARCH-001: 관리자OS ERP capability·`commerce_orders` vs `orders`/`payments`·정산 KPI·원장 분리 **코드 기준 포렌식**.  
21. **`docs/PLATFORM-ERP-DESIGN-001.md`** — PLATFORM-ERP-DESIGN-001: `commerce_orders` 최소 ERP 연결(`payments` SSOT·receivable owner·동기화 시점·migration 설계 목록만).  
22. **`supabase/migrations/20260515100000_add_commerce_order_id_to_payments.sql`** + **`updateCommerceOrderStatus`** — PLATFORM-ERP-P0-001: `paid` 확정 시 `payments` inbound + `commerce_order_id` (운영 DB 적용은 별도 승인).  
...
37. **`docs/ERP-ROADMAP-001.md`** — **ERP-ROADMAP-001**: 관리자OS ERP **실제 구현 로드맵** 재정의(D-021~024·transition debt·의존성 순서·위험 TOP5·운영 가능성 평가); **구현 아님**.
...
- **작업 이력 (2026-05-19)**: `payment.ts` — `PAYOUT_OUTBOUND_REVERSAL_BLOCKED_ERROR` export 제거(파일 내부 const) — worklog: [`docs/worklogs/2026-05-19_fix_payment-server-action-export.md`](./worklogs/2026-05-19_fix_payment-server-action-export.md)
- **작업 이력 (2026-05-19)**: `/automation/*` 빈 page → `/sales/*` page re-export, production build prerender 통과 — worklog: [`docs/worklogs/2026-05-19_fix_automation-export-prerender.md`](./worklogs/2026-05-19_fix_automation-export-prerender.md)
- **작업 이력 (2026-05-19)**: `(admin)/page` 루트 `/` 충돌 제거 → `/admin` redirect를 `(admin)/admin/page.tsx`로 이동, manifest ENOENT 해소 — worklog: [`docs/worklogs/2026-05-19_fix_admin-manifest-enoent.md`](./worklogs/2026-05-19_fix_admin-manifest-enoent.md)
...
- **작업 이력 (2026-05-14)**: `docs/TEST-DEV/TEST-RUN-001.md` 운영 테스트 **실행 가이드**(STEP 0~8·비가역·기록 템플릿; 코드 기준 경로만) — worklog: [`docs/worklogs/2026-05-14_docs_test-run-001-operational-guide.md`](./worklogs/2026-05-14_docs_test-run-001-operational-guide.md)
...
- **작업 이력 (2026-05-14)**: **TEST-RUN-ERP-001** — storefront→ERP **회계 숫자 정합성** 손 검증 가이드(`docs/TEST-DEV/TEST-RUN-ERP-001.md`) + `TEST.md`·`TEST-RUN-001.md`·`tasks.md` 연동 — worklog: [`docs/worklogs/2026-05-14_docs_test-run-erp-001-accounting-guide.md`](./worklogs/2026-05-14_docs_test-run-erp-001-accounting-guide.md)
- **작업 이력 (2026-05-14)**: **TEST-RUN-PRICING-001** — pricing engine P0 **손 검증 가이드**(`docs/TEST-DEV/TEST-RUN-PRICING-001.md`) + `TEST.md`·`TEST-RUN-ERP-001.md`·`tasks.md` 문서 사용법 연동 — worklog: [`docs/worklogs/2026-05-14_docs_test-run-pricing-001-guide.md`](./worklogs/2026-05-14_docs_test-run-pricing-001-guide.md)
...
- **작업 이력 (2026-05-14)**: **ERP-ROADMAP-001** ERP 실구현 우선순위·위험·운영 가능성 로드맵(`docs/ERP-ROADMAP-001.md`) · `tasks.md` Epic — worklog: [`docs/worklogs/2026-05-14_docs_erp-roadmap-001.md`](./worklogs/2026-05-14_docs_erp-roadmap-001.md)
...
- **세부 항목 분해 (Phase 5)**:
  ...
  - **[SUP-PARTIAL-001-F] 블록6 상품 TOP5 판매수량 컬럼 추가**
    ...
    - **작업 이력 (2026-05-19)**: 거래처 매출 TOP5 거래처명 미표시 — `customers` 배열 조인·CRM id fallback — worklog: `docs/worklogs/2026-05-19_hotfix_dashboard-top5-customer-name.md`
...
#### [SUP-MISSING-011] 상품 바코드 스캔 + 사진 인식 자동 등록 — **완료 (2026-05-08)**
- **PRODUCT 정의 위치**: PRODUCT §6-6 공급자OS 상품관리 + §8 식당OS 설정(식자재 입력 §4999~)
- **구현 방식 (확정)**:
  - **1순위**: 식품안전나라 OpenAPI **C005** (바코드연계제품정보)
  - **2순위**: **I2570** (유통바코드)
  - **3순위**: 공공데이터 **식품영양성분DB** (`FoodNtrCpntDbInfo02/getFoodNtrCpntDbInq02`) — 키는 `FOOD_NTR_API_KEY` 우선, 없으면 식품안전나라 키와 동일 값 사용
  - **4순위**: **Claude Vision** (`ANTHROPIC_API_KEY`, 모델 `ANTHROPIC_VISION_MODEL` 옵션)
  - **5순위**: 수동 입력
- **API 키**: 코드 하드코딩 금지 — 공급자OS는 `settings.foodsafety_api_key` → `FOOD_SAFETY_API_KEY` 폴백 / 식당OS는 `FOOD_SAFETY_API_KEY` 환경변수
- **라이브러리**: `@ericblade/quagga2` 카메라 스캔
- **대상 화면**:
  - realmyos: 상품 등록 폼 (`ProductCreateForm` + `BarcodeLookupSection`)
  - resturant_os: `/settings/ingredients` (`IngredientBarcodeSection`)
- **migration**: `products.barcode` / `ingredients.barcode` — 파일 추가 (`20260508150000_*`), **DB 적용은 별도 승인 후 실행**
- **작업 이력 (2026-05-08)**: worklog: [`docs/worklogs/2026-05-08_sup-missing-011_barcode-vision.md`](./worklogs/2026-05-08_sup-missing-011_barcode-vision.md)
...
- **작업 이력 (2026-05-14)**: **ERP-ROADMAP-001** — `docs/ERP-ROADMAP-001.md` 실구현 우선순위·위험·운영 가능성 로드맵 — worklog: [`docs/worklogs/2026-05-14_docs_erp-roadmap-001.md`](./worklogs/2026-05-14_docs_erp-roadmap-001.md)
...
- **작업 이력 (2026-05-14)**: **APPEND-ONLY-CONVERGENCE-DESIGN-001** outbound/storefront reversal 수렴 설계(`docs/APPEND-ONLY-CONVERGENCE-DESIGN-001.md`) — worklog: [`docs/worklogs/2026-05-14_docs_append-only-convergence-design-001.md`](./worklogs/2026-05-14_docs_append-only-convergence-design-001.md)
...
#### [ACCOUNTING-LIFECYCLE-DESIGN-001] settlement / payout / reversal chain lifecycle (설계 전용)
- **우선순위**: HIGH (payable·settlement·payout·KPI·finality 정렬)
- **상태**: **설계 문서만 (2026-05-14)** — 구현·migration 실행·DB 변경 없음
...
- **연계**: **`DECISIONS.md` [D-021]**, **`DECISIONS.md` [D-022]**, **`DECISIONS.md` [D-023]**, **[PAYMENTS-TAXONOMY-DESIGN-001](./PAYMENTS-TAXONOMY-DESIGN-001.md)**, **[PAYMENTS-TAXONOMY-POLICY-001]** (Epic), **[ACCOUNTING-LIFECYCLE-POLICY-001]** (Epic), **[ACCOUNTING-EVENT-MODEL-001](./ACCOUNTING-EVENT-MODEL-001.md)**, **[ACCOUNTING-REVERSAL-DESIGN-001](./ACCOUNTING-REVERSAL-DESIGN-001.md)**, **`[PLATFORM-ERP-001]`**, **[ACCOUNTING-REVERSAL-P0-001]**, **[KPI-REVERSAL-P0-001]**, [`docs/APPEND-ONLY-CONVERGENCE-DESIGN-001.md`](./APPEND-ONLY-CONVERGENCE-DESIGN-001.md) (**[APPEND-ONLY-CONVERGENCE-DESIGN-001]**)
...
- **우선순위**: HIGH (UPDATE vs INSERT 패턴 통일·[D-023] 이행)
- **상태**: **설계 문서만 (2026-05-14)** — 구현·migration 실행·DB 변경 없음
...
- **상태**: **로드맵 확정 (2026-05-14)** — 코드·migration·DB 변경 없음
- **산출물**: [`docs/ERP-ROADMAP-001.md`](./ERP-ROADMAP-001.md) — 완성도 %·남은 작업·의존성 순서·TOP5·transition debt P0~P2·지시 횟수 시나리오·운영 가능성 평가
- **연계**: **`[PLATFORM-ERP-001]`**, **`[APPEND-ONLY-CONVERGENCE-P1-001]`**, **`DECISIONS.md` [D-021]~[D-024]**, **[ACCOUNTING-LIFECYCLE-POLICY-001]**, **[ACCOUNTING-LIFECYCLE-DESIGN-001](./ACCOUNTING-LIFECYCLE-DESIGN-001.md)**, **`PLATFORM-ERP-ARCH-001`**, **`PLATFORM-ERP-DESIGN-001`**, **`docs/TEST-DEV/TEST-RUN-ERP-001.md`**
- **작업 이력 (2026-05-14)**: 로드맵 문서·`tasks.md`·worklog — worklog: [`docs/worklogs/2026-05-14_docs_erp-roadmap-001.md`](./worklogs/2026-05-14_docs_erp-roadmap-001.md)
...
#### [ACCOUNTING-REVERSAL-P0-001] append-only storefront reversal (최소 구현)
...
- **작업 이력 (2026-05-14)**: storefront 주문 멱등성 — `commerce_orders.checkout_submission_id`·`idempotency_key` 및 부분 유니크 인덱스 2종 — [`supabase/migrations/20260514200000_commerce_orders_idempotency.sql`](../supabase/migrations/20260514200000_commerce_orders_idempotency.sql) — worklog: [`docs/worklogs/2026-05-14_feat_commerce-order-idempotency.md`](./worklogs/2026-05-14_feat_commerce-order-idempotency.md)
...
- **작업 이력 (2026-05-10)**: `shipping_groups`·`commerce_product_listings.shipping_group_id` migration 소급 — [`supabase/migrations/20260510170000_create_shipping_groups.sql`](../supabase/migrations/20260510170000_create_shipping_groups.sql) — worklog: [`docs/worklogs/2026-05-10_feat_commerce-shipping-groups-listing-studio.md`](./worklogs/2026-05-10_feat_commerce-shipping-groups-listing-studio.md)
...
- **작업 이력 (2026-05-09)**: 상세 이미지 업로드 — append 직후 `Promise.resolve` 마이크로태스크가 React 커밋보다 먼저 돌아 `uploadDetailBlockFile`의 `prev.map`이 블록을 못 찾던 레이스 수정(`setTimeout(0)` 대기·순차 업로드·성공 시 블록 누락 append 복구, 오류 토스트는 updater 밖) — worklog: [`docs/worklogs/2026-05-09_hotfix_listing-detail-image-upload-state.md`](./worklogs/2026-05-09_hotfix_listing-detail-image-upload-state.md)
- **작업 이력 (2026-05-09)**: 상세 이미지 UX — `detailSavedCount`가 업로드 중(`url` 빈 문자열) 슬롯을 0으로 세어 운영자에게 “미등록”으로 보이던 문제 수정(`detailRegisteredCount`), `ingestDetailFiles` append를 `flushSync`로 커밋·업로드 시작 시 블록 upsert·성공 토스트·빈 목록 안내 문구 — worklog: [`docs/worklogs/2026-05-09_hotfix_listing-detail-preview-count-ui.md`](./worklogs/2026-05-09_hotfix_listing-detail-preview-count-ui.md)
- **작업 이력 (2026-05-09)**: 상세 이미지 피커 — `crypto.randomUUID` 비가용 환경 대비 `randomBlockId` 폴백, 다중 파일 `processIncomingDetailFiles` 단일 `flushSync` 일괄 append, 카드 위 파일 드롭 시 `stopPropagation`으로 부모 미수신 보완, 개발 전용 DEBUG 패널·카드 상태 문구·2장 이상일 때만 드래그 핸들 활성 — worklog: [`docs/worklogs/2026-05-09_hotfix_listing-detail-image-evidence-ui.md`](./worklogs/2026-05-09_hotfix_listing-detail-image-evidence-ui.md)
- **작업 이력 (2026-05-09)**: 상세 이미지 `lastFiles>0`·`blocks=0` — `validateImageFile`에서 전부 탈락 시 `pairs.length===0`로 `setDetailBlocks` 미호출이 원인임을 코드로 확정; MIME/확장자 허용 보강(JPG variants·GIF·BMP·AVIF·HEIC·octet-stream+확장자), 탈락 시 DEBUG·콘솔·요약 토스트, dev `DEBUG TEST BLOCK`·`detailBlocksRef` 로깅 — worklog: [`docs/worklogs/2026-05-09_debug_listing-detail-append-validation.md`](./worklogs/2026-05-09_debug_listing-detail-append-validation.md)
...
- **작업 이력 (2026-05-14)**: 상품 운영 구조 포렌식(`ListingNewClient`·create/update 분리·라우팅·재사용성)·장기 edit 설계 권고(코드 변경 없음) — worklog: [`docs/worklogs/2026-05-14_docs_listing-ops-forensic-design.md`](./worklogs/2026-05-14_docs_listing-ops-forensic-design.md)
...
- **작업 이력 (2026-05-14)**: `createCommerceOrder` 서버 멱등성(`checkout_submission_id`·페이로드 SHA256·유니크 충돌 시 기존 주문 반환, `commerce_order_duplicate_reused` 콘솔 로그) + 체크아웃 `crypto.randomUUID()` — worklog: [`docs/worklogs/2026-05-14_feat_commerce-order-idempotency.md`](./worklogs/2026-05-14_feat_commerce-order-idempotency.md) (`resturant_os`)
...
#### [DISCOUNT-FORENSIC-001] 기간할인 / 프로모션 가격 구조 포렌식 (문서)
- **우선순위**: HIGH (정무님 운영 정책 vs 실구현)
- **선행 조건**: 없음 (코드·migration·CONTEXT 인벤토리)
- **상태**: **완료 (2026-05-14)**
- **설명**: `commerce_price`/`original_price`·listing 기간 필드 부재·storefront `createCommerceOrder` 재가격·공급자 `orders.discount_amount` 수동 헤더·export 갭·고객별 타깃 부재를 확정.
- **산출물**: [`docs/DISCOUNT-FORENSIC-001.md`](./DISCOUNT-FORENSIC-001.md)
- **migration 필요**: NO
- **작업 이력 (2026-05-14)**: 문서·문서 사용법 14·OPS·본 블록 — worklog: [`docs/worklogs/2026-05-14_docs_discount-forensic-001-pricing.md`](./worklogs/2026-05-14_docs_discount-forensic-001-pricing.md)

#### [DISCOUNT-ENGINE-DESIGN-001] B2B 가격정책 엔진 설계 (문서)
- **우선순위**: HIGH (ERP 금액 분리·부담 주체·스냅샷)
- **선행 조건**: `DISCOUNT-FORENSIC-001`·`PLATFORM-ERP-P2-003` allocation/payable 현행
- **상태**: **설계 문서 (2026-05-14)** — 구현 아님
- **설명**: `customer_charge` / `supplier_basis` / 부담 주체별 할인 / `platform_fee` / `supplier_payable` 분리, 우선순위·기간 프로모션·스냅샷 시점·ERP(`payments`·allocation·`supplier_payables`) 연동 방향만 정의.
...
- **작업 이력 (2026-05-14)**: **TEST-RUN-PRICING-001** P0 가격 엔진 **손 검증 가이드**(`docs/TEST-DEV/TEST-RUN-PRICING-001.md`)·`TEST.md`·`TEST-RUN-ERP-001` 연동 — worklog: [`docs/worklogs/2026-05-14_docs_test-run-pricing-001-guide.md`](./worklogs/2026-05-14_docs_test-run-pricing-001-guide.md)
...
#### [LAUNCH-001] Supabase Pro 플랜 업그레이드 — **필수**
- **판정**: **CRITICAL** (유료 서비스 출시 전 필수)
- **현재**: Free 플랜 (백업 없음)
- **필요**: Pro 플랜 (\$25/월)
- **포함**: 일별 자동 백업 7일 보관
- **URL**: `https://supabase.com/dashboard/project/cqiwcyuclpuarynrreat/settings/billing`
- **이유**: 주문/수금/미수/원장 등 **돈 데이터**가 핵심이며, **백업 없으면 장애 시 복구 불가**

#### [LAUNCH-002] UptimeRobot 모니터링 설정 — **필수**
- **판정**: **HIGH** (유료 서비스 출시 전 필수)
- **URL**: `https://uptimerobot.com`
- **설정**: 공급자OS + 식당OS Vercel URL 등록
- **간격**: 5분마다 체크
- **알림**: 이메일 또는 카카오
