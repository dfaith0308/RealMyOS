...
interface Props {
  ...
  filters: { from: string; to: string; customer_id: string; status: string; period?: string }
}
...
export default function PaymentsClient({ payments, customers, filters }: Props) {
  ...
  function pushWith(next: typeof localFilters) {
    ...
    if (next.customer_id) params.set('customer_id', next.customer_id)
            ...
            {payments.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  {totalDeposit > 0 && (
                    <td
                      style={{
                        ...
                        textAlign: 'right',
                        fontVariantNumeric: 'tabular-nums',
                        color: p.deposit_amount > 0 ? '#1D4ED8' : '#d1d5db',
                        fontSize: 12,
                      }}
                    >
                  ...
                  <td style={td}>
                    {p.status === 'confirmed' && (
                      <button style={s.cancelBtn} onClick={() => setCancelTarget(p)}>
                        취소
                      </button>
                    )}
                  </td>
        ...
        <div style={s.overlay} onClick={() => setCancelTarget(null)}>
          <div style={s.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ fontSize: 13, color: '#374151', margin: '0 0 16px 0', lineHeight: 1.6 }}>
              {cancelTarget.customer_name} — {formatKRW(cancelTarget.amount)}
              <br />({cancelTarget.payment_date})
              <br />
              취소하시겠습니까?
            </p>
            <div style={{ display: 'flex', gap: 8 }}>
              <button style={s.modalCancelBtn} onClick={() => setCancelTarget(null)}>
                아니오
              </button>
              <button
                style={isPending ? s.modalConfirmOff : s.modalConfirmBtn}
