...
export async function getExperiments(): Promise<ActionResult<ExperimentRow[]>> {
  const supabase = await createSupabaseServer()
  const auth = await requireAdmin(supabase)
  if (!auth.ok) return { success: false, error: auth.error }
  ...
  const { data, error } = await supabase
    .from('admin_settings')
    .select('key, value')
    .like('key', 'experiment\\_%')
    .order('key', { ascending: true })
    .limit(500)

  if (error) return { success: false, error: error.message }

  const m = new Map((data ?? []).map((r: any) => [String(r.key), String(r.value ?? '')]))
  const names = new Set<string>()
  for (const k of m.keys()) {
    const mm = k.match(/^experiment_(.+)_(a|b|start|end)$/)
    if (mm?.[1]) names.add(mm[1])
  }

  const out: ExperimentRow[] = [...names].sort().map((name) => ({
    name,
    a_value: m.get(`experiment_${name}_a`) ?? '',
    b_value: m.get(`experiment_${name}_b`) ?? '',
    start_date: m.get(`experiment_${name}_start`) ?? null,
    end_date: m.get(`experiment_${name}_end`) ?? null,
  }))

  await insertAdminLog(supabase, {
    admin_id: auth.ctx.user_id,
    action_type: 'experiments_view',
    target_table: 'admin_settings',
  }).catch(() => {})
...
export async function startExperiment(input: {
  ...
  if (!name) return { success: false, error: '실험명(name)을 입력해 주세요. (영문/숫자/_/-)' }
  if (!input.a_value?.trim() || !input.b_value?.trim()) return { success: false, error: '정책 A/B 값을 입력해 주세요.' }

  const keys = [
    { key: `experiment_${name}_a`, value: input.a_value.trim() },
    { key: `experiment_${name}_b`, value: input.b_value.trim() },
    { key: `experiment_${name}_start`, value: (input.start_date ?? '').trim() },
    { key: `experiment_${name}_end`, value: (input.end_date ?? '').trim() },
  ]

  const nowIso = new Date().toISOString()

  // upsert (admin_settings는 key unique)
  const { data: before } = await supabase
    .from('admin_settings')
    .select('key, value')
    .in('key', keys.map((k) => k.key))

  const beforeMap = new Map((before ?? []).map((r: any) => [String(r.key), String(r.value ?? '')]))

  const { error } = await supabase.from('admin_settings').upsert(
    keys.map((k) => ({
      key: k.key,
      value: k.value,
      description: '실험 정책 값 (FORENSIC-002-C)',
      updated_at: nowIso,
      updated_by: auth.ctx.user_id,
    })),
    { onConflict: 'key' },
  ...
  const logRes = await insertAdminLog(supabase, {
    ...
    target_table: 'admin_settings',
    reason: 'FORENSIC-002-C',
    old_value: { name, before: Object.fromEntries(beforeMap) },
    new_value: { name, keys },
  })
  if (!logRes.ok) return { success: false, error: `admin_logs 기록 실패: ${logRes.error}` }
